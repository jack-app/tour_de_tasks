...
class UserData {
  ...
  Future<void> prepare() async {
    prefs ??= await SharedPreferencesWithCache.create(cacheOptions: 
  ...
  // 開始地点
  String get startCity => prefs!.getString('startCity')??cities[0];
  ...
  double get confPassedDistanceKm => prefs!.getDouble('confPassedDistanceKm')??0.0;
  set confPassedDistanceKm(double value) => prefs!.setDouble('confPassedDistanceKm', value);
...
class LapRepository {
  ...
  Future<List<Lap>?> get(int afterEpochSec) async {
    var results = await db!.query(
      tableName, 
      where: 'whenEpochSec > ?', 
      whereArgs: [afterEpochSec]
    );
    if (results.isEmpty) return null;
    return results.map((e) => Lap(
      whenEpochSec: e['whenEpochSec'] as int, 
      act: e['act'] as String
    )).toList();
  }
